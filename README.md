...
Tabulation System for Missus Nabua (Nabua, Camarines Sur)
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/mrsnabua`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `mrsnabua` and import [mrsnabua.sql](mrsnabua.sql) into it.

6. Open the terminal and navigate to the project directory **mrsnabua**.
...
9. Open your web browser and access <http://localhost:5200/mrsnabua/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-mrsnabua` and import [mrsnabua.sql](mrsnabua.sql) into it.
...
2. Access the application by visiting `http://[host_name]/mrsnabua`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/mrsnabua>
     - <http://192.168.1.99/mrsnabua>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/mrsnabua/app>.
